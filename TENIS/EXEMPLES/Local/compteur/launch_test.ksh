...
if [[ ( "$1" == "NO_TEST" ) || ( $1 -eq '1' ) ]]
    then
    clean_all_useless_file
    compile src/main.c src/counter.c
    bin/prog
...
then
   ...
   # sed -i 's/end/end\nrun/4' /tmp/user_debug.gdb	
    compile /tmp/maintenis_tmp.c src/counter.c
    run_TENIS_script_GDB
    ${TENIS_POST_PRO} test_result.log
    ${READER} rapport_test.txt

    ## Lance un test en utilisant TENIS avec couverture de code du fichier src/counter.c
elif [[ ( "$1" == "COUV_math" ) || ( $1 -eq '5' ) ]]
then
    clean_all_useless_file
    generate_prepro_file src/counter.c
    ${TENIS} ${TEST_PTU} -c src/counter.c.out
    compile /tmp/maintenis_tmp.c src/counter.c
...
then
    ...
    echo "5) COUV_counter.c: test avec couverture de code de src/counter.c"
