...
if [[ ( "$1" == "NO_TEST" ) || ( $1 -eq '1' ) ]]
    then
    clean_all_useless_file
    compile src/stat.c src/main.c src/math.c 
    bin/prog
...
then
   ...
   # sed -i 's/end/end\nrun/4' /tmp/user_debug.gdb	
    compile src/stat.c /tmp/maintenis_tmp.c src/math.c
    run_TENIS_script_GDB
...
then
	set -x  
    clean_all_useless_file
    generate_prepro_file src/math.c
    ${TENIS} ${TEST_PTU} -c src/math.c.out
    compile src/stat.c /tmp/maintenis_tmp.c src/math.c
